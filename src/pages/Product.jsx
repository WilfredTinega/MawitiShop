    ...
    <div className='border-t-2 pt-10 transition-opacity ease-in duration-500 opacity-100'>
            ...
            <div className='flex gap-2'>
              {
                productData.sizes.map((item, index)=>(
                  <button onClick={()=>setSize(item)} className={`border border-gray-200 rounded-sm py-2 px-4 bg-gray-100 cursor-pointer ${item === size ? "border-orange-500":""}`} key={index}>{item}</button>
                ))
              }
            </div>
          </div>
      ...
      <div className="mt-20">
        <div className="flex ">
          ...
          <p className="border border-gray-500 px-5 py-3 text-sm">Reviews (122)</p>
        </div>
        <div className="flex flex-col gap-4 border px-6 py-6 tex-sm text-gray-500">
          ...
          <p>A typical eCommerce website includes features like product listings with images and descriptions, a shopping cart for users to add items, and a secure checkout process with payment gateway integration. It also supports user authentication for account creation, order tracking, and personalized experiences. Advanced functionalities such as search filters, reviews and ratings, and mobile responsiveness enhance the user experience. </p>
        </div>
      </div>
